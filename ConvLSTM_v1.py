...
from tensorflow.keras.layers import LSTM, Dense, Flatten, Conv1D, ConvLSTM2D
...
# ============================================================各种自定义参数============================================================
# ===================================================================================================================================
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # 选择运行GPU
...
def modelconstruction(use_step, pre_step, num_classes, sellect_compile):
    ...
    # Model 3
    model3 = Sequential()
    model3.add(ConvLSTM2D(64, activation='relu', kernel_size=(3, 3), input_shape=(use_step, 1, 34, 1), padding='same', return_sequences=True))
    model3.add(ConvLSTM2D(32, activation='relu', kernel_size=(3, 3), padding='same'))
    model3.add(Flatten())
    model3.add(Dense(units=pre_step*34, activation='linear'))
...
def get_train_batch(datalist, batch_size):
    ...
    while 1:
        for i in range(0, len(datalist), batch_size):
            x = (x_data[i:i + batch_size]).reshape(-1, use_step, 1, 34, 1)
            ...
            yield ({'conv_lstm2d_input': x}, {'dense': y})
...
root_p = 'convlstm_{}step_{}'.format(pre_step, usefunction)
...
modelname = os.path.join("checkpoint", root_p, 'convlstm.h5')
batch_size = 128
epochs = 20
num_classes = 34
input_shape = (use_step, 1, 34, 1)
...
# np.savetxt(os.path.join(folder_path, 'convlstm_loss.csv'), history.history['loss'], delimiter=",")
# np.savetxt(os.path.join(folder_path, 'convlstm_val_loss.csv'), history.history['val_loss'], delimiter=",")
# plt.legend()
# plt.savefig(os.path.join(folder_path, 'convlstm_loss.png'))
# plt.show()
...
y_pred = scaler.inverse_transform(model.predict(test_feat.reshape(-1, use_step, 1, 34, 1)).reshape(-1, 34)).reshape(-1, pre_step, 34)
